// Verify: https://judge.u-aizu.ac.jp/onlinejudge/review.jsp?rid=6357906
fn main() {
  input! {
    n: usize, m: usize,
  ...
  let mut graph = MaxFlowEdmondsKarp::new(n);
  for &(u, v, c) in &arcs {
    graph.add_arc(u, v, c);
  }
...
impl MaxFlowEdmondsKarp {
  ...
  pub fn flow_limited(&mut self, source: usize, sink: usize, limit: u64) -> u64 {
    ...
    loop {
      ...
      visited[source] = true;
      queue.push_back((source, limit - flow_sum));
      while let Some((u, cap)) = queue.pop_front() {
        for &arc_id in &self.graph[u] {
          ...
          prev[arc.to] = arc_id;
          caps[arc.to] = new_cap;
          queue.push_back((arc.to, new_cap));
        }
      }
      if !visited[sink] {
        break;
