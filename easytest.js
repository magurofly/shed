...
(() => {
	...
	// get elements
	const container = $("#task-statement > .lang > span[style]");
	...
	const sourceCodeTextarea = submissionForm.find("[name='sourceCode']");
	const csrfToken = submissionForm.find("[name='csrf_token']").val();

	// utils
	const getBody = input => {
		submissionForm[0].dispatchEvent(new SubmitEvent("submit"));
		const languageId = encodeURIComponent(languageIdSelect.val());
		const sourceCode = encodeURIComponent(sourceCodeTextarea.val());
...
<details id="testcase${i}-details" class="row">
	...
	const testButton = $(`<a class="btn btn-secondary">Run Test</a>`);
