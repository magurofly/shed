...
trait MyItertools : Iterator + Sized {
  ...
  fn mex(self, g: impl IntoIterator<Item = Self::Item>) -> Self::Item where Self::Item: Ord { let mut v = self.collect::<Vec<_>>(); v.sort(); v.dedup(); let mut it = v.into_iter(); g.into_iter().find(|a| if let Some(x) = it.next() { a != &x } else { true }).unwrap() }
}
